...
const { generatePatientUniqueIdentifier } = require("./generalMixin");
const matchAutoURI = URI("http://openclientregistry.org/fhir")
  .segment("automatch")
  .toString();
const matchIssuesURI = URI("http://openclientregistry.org/fhir")
  .segment("matchIssues")
  .toString();
const reprocessingURI = URI("http://openclientregistry.org/fhir")
  .segment("require-reprocess")
  .toString();
const humanAdjudURI = URI("http://openclientregistry.org/fhir")
  .segment("humanAdjudication")
  .toString();
...
function closeCSVAuditEvent(event) {
  ...
  if (Array.isArray(eventCopy.extension)) {
    for (let extInd in eventCopy.extension) {
      if (
        eventCopy.extension[extInd].url ===
        "http://openclientregistry.org/fhir/extension/csvauditreport"
      ) {
...
function createCSVUploadAudEvent(operSummary, auditBundle, req) {
  ...
  if (processing || saving) {
    return new Promise((resolve, reject) => {
      setTimeout(() => {
        createCSVUploadAudEvent(operSummary, auditBundle, req)
          .then(() => {
            resolve();
          })
          .catch((err) => {
      ...
      for (let autoIndex in operSummary.FHIRAutoMatches) {
        for (let esmatch of operSummary.ESMatches) {
          for (let esauto of esmatch.autoMatchResults) {
            if (
              esauto._id === operSummary.FHIRAutoMatches[autoIndex].resource.id
            ) {
              operSummary.FHIRAutoMatches[autoIndex].score = esauto._score;
              operSummary.FHIRAutoMatches[autoIndex].threshold =
                esmatch.rule.autoMatchThreshold;
            }
          }
        }
      }
      ...
      for (let autoIndex in operSummary.FHIRPotentialMatches) {
        for (let esmatch of operSummary.ESMatches) {
          for (let esauto of esmatch.potentialMatchResults) {
            if (
              esauto._id ===
              operSummary.FHIRPotentialMatches[autoIndex].resource.id
            ) {
              operSummary.FHIRPotentialMatches[autoIndex].score = esauto._score;
              operSummary.FHIRPotentialMatches[autoIndex].threshold =
                esmatch.rule.potentialMatchThreshold;
            }
          }
        }
      }
      ...
      async.series(
          ...
          (callback) => {
            eventIndexInBundle = auditBundle.entry.findIndex((entry) => {
              return (
                entry.resource.id ===
                uuid5(
                  operSummary.csvCode.toString(),
                  "00b3ffab-450c-4407-9e59-05034a271da7"
                )
              );
            });
        ...
        () => {
          if (!event) {
            ...
            crCache.set(
              `csvreport_${operSummary.csvCode}`,
            ...
            if (csvTag && csvTag.code === operSummary.csvCode) {
              for (let index in event.entity) {
                ...
                if (entity.what.reference === `Patient/${auto.resource.id}`) {
                  ...
                  let exist =
                    details.autoMatches &&
                    details.autoMatches.find((mtch) => {
                      return (
                        mtch.resource.id === operSummary.submittedResource.id
                      );
                    });
                  if (exist) {
                    continue;
                  ...
                  event.entity[index].detail[detIndex].valueString =
          ...
          for (let potential of operSummary.FHIRPotentialMatches) {
            ...
            if (csvTag && csvTag.code === operSummary.csvCode) {
              for (let index in event.entity) {
                let entity = event.entity[index];
                if (
                  entity.what.reference === `Patient/${potential.resource.id}`
                ) {
                  let detIndex = entity.detail.findIndex((det) => {
                    return det.type === "match";
                  });
                  ...
                  let exist =
                    details.potentialMatches &&
                    details.potentialMatches.find((mtch) => {
                      return (
                        mtch.resource.id === operSummary.submittedResource.id
                      );
                    });
  ...
  for (const operSummary of operationSummary) {
    ...
    if (operSummary.submittedResource) {
      const submRes = {
        ...
        what: {
          reference:
            operSummary.submittedResource.resourceType +
            "/" +
            operSummary.submittedResource.id,
        },
        detail: [
          {
            type: "resource",
...
const addPatient = (clientID, patientsBundle, callback) => {
  let autoMatchPatientWithHumanAdjudTag = config.get(
    "matching:autoMatchPatientWithHumanAdjudTag"
  );
      ...
      async ({
        ...
        for (const auto of FHIRAutoMatched.entry) {
          let isCurrentLink = currentLinks.find((currentLink) => {
            return auto.resource.link.find((link) => {
              return (
                ...
                currentLink.resource.resourceType +
                  "/" +
                  currentLink.resource.id
              );
            });
          });
          ...
          if (
            isCurrentLink &&
            hasHumanAdjudTag &&
            !autoMatchPatientWithHumanAdjudTag
          ) {
            continue;
          } else {
        ...
        if (existsPotentialMatches) {
          ...
          let tagExist = patient.meta.tag.find((tag) => {
            return (
              tag.system === matchIssuesURI && tag.code === "potentialMatches"
            );
          });
          if (!tagExist) {
            patient.meta.tag.push({
          ...
          for (let tagIndex in patient.meta.tag) {
            ...
            if (
              tag.system === matchIssuesURI &&
              tag.code === "potentialMatches"
            ) {
        ...
        if (existsConflictMatches) {
          ...
          let tagExist = patient.meta.tag.find((tag) => {
            return (
              tag.system === matchIssuesURI && tag.code === "conflictMatches"
            );
          });
          if (!tagExist) {
            patient.meta.tag.push({
        ...
        if (
          ...
          matchedGoldenRecords.entry.length === 0
        ) {
          let goldenRecord;
          ...
          existLinkPromise.then(() => {
            ...
            responseHeaders.CRUID.push(
              goldenRecord.resourceType + "/" + goldenRecord.id
            );
            ...
            const promise = new Promise((resolve, reject) => {
              if (newPatient) {
                ...
                fhirWrapper.saveResource(
                  ...
                  (err, body) => {
                    responseBundle.entry = responseBundle.entry.concat(
                      body.entry
                    );
                    ...
                    if (csvTag && csvTag.code && body && body.entry) {
                      ...
                      for (let entry of body.entry) {
                        if (
                          parseInt(entry.response.etag) > latestHistory &&
                          entry.response.location.startsWith(
                            `Patient/${patient.id}/_history`
                          )
                        ) {
                    ...
                    if (err) {
                      ...
                      operSummary.outcomeDesc =
                        "An error occured while saving patient and golden record";
          ...
          if (hasHumanAdjudTag && !autoMatchPatientWithHumanAdjudTag) {
            ...
            if (currentLinks.length > 0) {
              ...
              for (const currentLink of currentLinks) {
                const exist =
                  currentLink.resource.link &&
                  currentLink.resource.link.find((link) => {
                    return link.other.reference === "Patient/" + patient.id;
                  });
                let replacedByNewGolden = false;
                if (
                  currentLink.resource.link &&
                  currentLink.resource.link.length === 1 &&
                  exist
                ) {
                  const inNewMatches = matchedGoldenRecords.entry.find(
                    (entry) => {
                ...
                for (const index in currentLink.resource.link) {
                  if (
                    currentLink.resource.link[index].other.reference ===
                    "Patient/" + patient.id
                  ) {
                    // remove patient from golden link
                    if (replacedByNewGolden) {
                      currentLink.resource.link[index].other.reference =
                        "Patient/" + matchedGoldenRecords.entry[0].resource.id;
                    ...
                    for (const index in patient.link) {
                      if (
                        patient.link[index].other.reference ===
                        "Patient/" + currentLink.resource.id
                      ) {
            ...
            for (const goldenRecord of matchedGoldenRecords.entry) {
              operSummary.cruid.push(
                goldenRecord.resource.resourceType +
                  "/" +
          ...
          operSummary.outcome = "8";
          operSummary.outcomeDesc =
            "Invalid response returned when attempted to get matches";
          logger.error(
            "Invalid response returned when attempted to get matches"
          );
  ...
  async.eachSeries(
    ...
    (newPatient, nxtPatient) => {
      ...
      const tagExist =
        ...
        newPatient.resource.meta.tag.find((tag) => {
      ...
      if (!tagExist) {
        ...
        newPatient.resource.meta.lastUpdated = moment().format(
          "YYYY-MM-DDTHH:mm:ss.SSSZ"
        );
        newPatient.resource.meta.tag.push({
          system: URI(config.get("systems:CRBaseURI"))
            .segment("clientid")
            .toString(),
          code: clientID,
          display: clientName,
      ...
      if (!internalIdURI || internalIdURI.length === 0) {
        operSummary.outcome = "8";
        operSummary.outcomeDesc =
          "URI for internal id is not defined on configuration files";
        logger.error(
          "URI for internal id is not defined on configuration files, stop processing patient"
        );
        operationSummary.push(operSummary);
      ...
      if (!validSystem) {
        operSummary.outcome = "4";
        operSummary.outcomeDesc =
          "Patient resource has no identifier for internalid registered by client registry";
        operationSummary.push(operSummary);
        logger.error(
          "Patient resource has no identifier for internalid registered by client registry, stop processing"
        );
        deleteProcess(processID);
        return nxtPatient();
      }
      ...
      fhirWrapper.getResource(
        ...
        (patientData) => {
          ...
          if (existingPatients.length === 0) {
                ...
                fhirWrapper.saveResource(
                  ...
                  (err, body) => {
                    if (err) {
                      operSummary.outcome = "8";
                      operSummary.outcomeDesc =
                        "An error occured while saving a bundle that contians matches of a submitted resource and the submitted resource itself";
                      operationSummary.push(operSummary);
                      deleteProcess(processID);
                      return nxtPatient();
                    }
                    ...
                    operSummary.what =
                      newPatient.resource.resourceType +
                      "/" +
                      newPatient.resource.id;
                      ...
                      cacheFHIR.fhir2ES(
                        ...
                        (err) => {
                          if (err) {
                            operSummary.outcome = "8";
                            operSummary.outcomeDesc =
                              "An error has occured while caching patient changes into elasticsearch";
                          }
                          operationSummary.push(operSummary);
                          deleteProcess(processID);
                          return nxtPatient();
            ...
            operSummary.what =
              existingPatient.resource.resourceType +
              "/" +
              existingPatient.resource.id;
            logger.info(
              `Patient ${JSON.stringify(
                newPatient.resource.identifier
              )} exists, updating database records`
            );
            ...
            let adjudTag =
              existingPatient.resource.meta &&
              existingPatient.resource.meta.tag &&
              existingPatient.resource.meta.tag.find((tag) => {
                return (
                  tag.system === humanAdjudURI &&
                  tag.code === "humanAdjudication"
                );
              });
            async.series(
              [
                ...
                (callback) => {
                  ...
                  if (
                    !adjudTag ||
                    (adjudTag && autoMatchPatientWithHumanAdjudTag)
                  ) {
                    delete existingPatient.resource.link;
                  }
                  existingPatient.resource = _.merge(
                    existingPatient.resource,
                    newPatient.resource
                  );
                ...
                (callback) => {
                  ...
                  for (const goldenRecord of goldenRecords) {
                    for (const index in goldenRecord.resource.link) {
                      if (
                        goldenRecord.resource.link[index].other.reference ===
                        link
                      ) {
                        ...
                        if (csvTag && csvTag.code && body && body.entry) {
                          ...
                          for (let entry of body.entry) {
                            if (
                              parseInt(entry.response.etag) > latestHistory &&
                              entry.response.location.startsWith(
                                `Patient/${existingPatient.resource.id}/_history`
                              )
                            ) {
                              patientHistory = entry.response.location;
                            }
                              ...
                              entity: [
                                {
                                  role: "source",
                                  what: {
                                    reference: `DocumentReference/${csvTag.code}`,
                                  },
                                },
                              ],
                            };
    ...
    fhirWrapper.getResource(
      ...
      (patients) => {
        ...
        async.eachSeries(
          ...
          (patient, nxtPatient) => {
            let client =
              patient.resource.meta &&
              patient.resource.meta.tag &&
              patient.resource.meta.tag.find((tag) => {
                return (
...
const getCSVTag = (patient) => {
  let csvTag =
    patient.meta &&
    patient.meta.tag &&
    patient.meta.tag.find((tag) => {
      return (
        tag.system ===
        URI(config.get("systems:CRBaseURI"))
          .segment("tag")
          .segment("csv")
          .toString()
      );
